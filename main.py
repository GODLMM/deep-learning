...
with tf.variable_scope('Decoder',reuse=False):
    ...
    fc3=tf.layers.dense(fc2,units=image_size*image_size*channel,activation=tf.nn.relu,name='FC3')
...
for epoch in range(epochs):
    ...
    for i in range(idx):
        ...
        if (i+1)%500==0:
            print('500 batchs finished!')
    train_dict={X_train:x_train,y:y_train}
    val_dict={X_train:x_val,y:y_val}
    Total_loss,M_loss,R_loss,train_acc=sess.run([total_loss,m_loss,r_loss,acc],feed_dict=train_dict)
    val_acc=sess.run(acc,feed_dict=val_dict)
    val_decoder=sess.run(decoder_outputs,feed_dict={X_train:x_val[:100],y:y_val[:100]})
    #保存重构图片
    tl.visualize.save_images(val_decoder,[10,10],'images/val_{:03d}.jpg'.format(epoch))
    print('epoch: %d, train_loss: %.4f,train_acc: %.3f,val_acc: %.3f'%(epoch,Total_loss,train_acc,val_acc))
